title: 'ColaboraPANC'
subtitle: 'Internal editorial draft for manuscript preparation (not the official journal template)'
tags:
...
Editorial status
This file is an internal editorial draft for manuscript preparation. It consolidates agreed metadata and manuscript content, but it is not the official journal submission template.
...
Georeferenced collaborative registration of PANC observations.

AI-assisted plant identification support.

Expert validation workflow with auditable history.

Taxonomic enrichment from external providers.

Environmental and territorial monitoring integrations.

Mobile/web parity with API-backed workflows.
...
Technical metadata table (C1–C9)
...
Scientific workflow from citizen record submission to expert-validated output.

External integrations and enrichment pipeline.

Representative web/mobile interface or dashboard.

Auditable AI suggestion versus expert validation trail.
